...
class TestTelegramBot:
    ...
    async def test_main_with_valid_env(self):
        ...
        with patch.dict(
            os.environ,
            {"TELEGRAM_BOT_TOKEN": "test_token", "DATABASE_URL": "sqlite+aiosqlite:///:memory:"},
        ):
            with (
                patch("src.presentation.telegram.bot.Bot") as mock_bot_class,
                patch("src.presentation.telegram.bot.Dispatcher") as mock_dp_class,
                patch("src.presentation.telegram.bot.get_database") as mock_get_db,
