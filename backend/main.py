...
async def start_bot():
    ...
    КРИТИЧНО: Эта функция должна вызываться ТОЛЬКО ОДИН РАЗ.
    Используется глобальный флаг и lock для предотвращения повторного запуска polling.
    
    При TelegramConflictError:
    - Polling останавливается
    - Сессия бота закрывается
    ...
    async with _bot_polling_lock:
        if _bot_polling_started:
            logger.warning(
                "⚠️  Попытка повторного запуска Telegram бота предотвращена. "
                "Polling уже запущен. Это может указывать на проблему в коде."
            )
            return
    ...
    _bot_instance = bot  # Сохраняем для корректного завершения при конфликте
    ...
    conflict_occurred = False  # Флаг для отслеживания конфликта (логируем только один раз)
    
    @dp.errors()
    async def error_handler(update, exception):
        """Обработчик ошибок для перехвата конфликтов."""
        nonlocal conflict_occurred
        if isinstance(exception, TelegramConflictError):
            if not conflict_occurred:  # Логируем только один раз
    ...
    try:
        ...
        # КРИТИЧНО: Обработка конфликта при старте polling
        # Логируем только один раз, без повторных попыток
        logger.error(
            "⚠️ КОНФЛИКТ при запуске polling: Другой экземпляр бота уже запущен с тем же токеном.\n"
            f"Ошибка: {e}\n"
            "Возможные решения:\n"
            "1. Остановите другие экземпляры бота (локальные или на других серверах)\n"
            "2. Используйте webhook вместо polling (настройте через переменные окружения)\n"
        ...
        try:
            ...
            logger.debug(f"Ошибка при закрытии сессии: {close_error}")
        # Сбрасываем флаг для возможности перезапуска (если нужно)
        _bot_polling_started = False
        return
    except Exception as e:
        ...
        # Обработка конфликта с другим экземпляром бота (fallback для других типов ошибок)
        if "Conflict" in error_message or "terminated by other getUpdates" in error_message:
            ...
            try:
                ...
                logger.debug(f"Ошибка при закрытии сессии: {close_error}")
            # Сбрасываем флаг для возможности перезапуска
            _bot_polling_started = False
            return
...
async def main():
    ...
    # КРИТИЧНО: Запускаем бота в отдельной задаче, чтобы он работал параллельно с веб-сервером
    # Polling блокирует выполнение, поэтому запускаем его в фоне
    # Важно: бот запускается ТОЛЬКО ОДИН РАЗ, только здесь, только если включен
    enable_bot = os.getenv("ENABLE_TELEGRAM_BOT", "true").lower() in ("true", "1", "yes")
    ...
    if enable_bot:
        ...
        # Проверяем, что задача не завершилась сразу (это означало бы ошибку)
        if bot_task.done():
            error = bot_task.exception()
            if error:
                logger.error(f"❌ Бот не запущен: {error}")
                logger.warning("⚠️  Веб-сервер продолжает работать независимо от бота")
            else:
                logger.warning("⚠️  Бот завершился сразу после запуска")
                logger.warning("⚠️  Веб-сервер продолжает работать независимо от бота")
        else:
            logger.info("✅ Telegram бот успешно запущен и работает в фоновом режиме")
    else:
    ...
    # Бот работает параллельно в bot_task
    try:
        await web_task
    except Exception as e:
        logger.error(f"❌ Веб-сервер завершился с ошибкой: {e}")
        # Отменяем задачу бота, если веб-сервер завершился
